function run_tc2(grid_connected)
%RUN_TC2  Runs OPF-2 for TC2 (modified IEEE 14-bus DC system)

clc; close all;

if nargin < 1 || isempty(grid_connected)
    fprintf('\nSelect grid connection mode for TC2:\n');
...
fprintf('\nSet PV availability level for TC2 (0 to 1):\n');
...
sys = build_tc2_14bus(grid_connected);
sys.load_sheet = 'Test Case 2';
...
function [pv_prof, load_prof, sys] = get_profiles_from_csv_or_default(sys, T)
    ...
    if exist('read_pv_and_load_csv','file') == 2
        try
            ...
            warning('%s', sprintf('[run_tc2] Profile file load failed. Falling back to generated profiles.\n%s', ME.message));
        end
    end
